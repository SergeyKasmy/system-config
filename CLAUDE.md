...
Key conventions:
- **Never** use `hl.exec_cmd("hyprctl dispatch ...")`. Use `hl.dispatch(hl.dsp.something(...))` directly — `hyprctl dispatch` won't work with the Lua config.
- `exec(cmd)` is for small scripts; `exec_app(app)` wraps the configured app with a launcher for GUI apps.
